...
class GitLab:
    ...
    def add_user_to_group(self, user_id: int, group_id: int, role: int):
        ...
        rosim = self.gl.groups.get(group_id)
        match role:
            case 10:
                rosim.members.create({'user_id': user_id, 'access_level': gitlab.GUEST_ACCESS})
            case 20:
                rosim.members.create({'user_id': user_id, 'access_level': gitlab.REPORTER_ACCESS})
            case 30:
                rosim.members.create({'user_id': user_id, 'access_level': gitlab.DEVELOPER_ACCESS})
            case 40:
                rosim.members.create({'user_id': user_id, 'access_level': gitlab.MAINTAINER_ACCESS})
            case _:
